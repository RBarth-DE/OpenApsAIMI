...
internal class UnscentedKalmanFilterPluginTest {
    ...
    /**
     * The plugin reads IOB through a provider, which is a plain function type since the Metro
     * migration removed Dagger's `Provider`.
     *
     * Most tests here were written before the compression gate existed, so the default reports a
     * high total: the gate stays out of the way and the smoothing result is what it always was.
    ...
    private fun iobProvider(bolusIob: Double, basalIob: Double = 0.0): () -> IobCobCalculator = {
        mock<IobCobCalculator> {
            on { calculateIobFromBolus() } doReturn IobTotal(time = 0L, iob = bolusIob)
            on { calculateIobFromTempBasalsIncludingConvertedExtended() } doReturn IobTotal(time = 0L, iob = basalIob)
        }
    }
    ...
    @Test fun `a compression low with near-zero IOB is damped, not tracked to the floor`() {
        ...
        val damped = runBlocking { plugin(iobProvider(bolusIob = 0.1)).smooth(series(*vals), SmoothingContext.NONE) }[0].smoothed!!
        // With real insulin on board (guard disabled) the same fall IS followed down.
        val followed = runBlocking { plugin(iobProvider(bolusIob = 3.0)).smooth(series(*vals), SmoothingContext.NONE) }[0].smoothed!!
    ...
    @Test fun `negative basal IOB does not arm the compression gate`() {
        ...
        val iob = iobProvider(bolusIob = 3.0, basalIob = -5.0)
        val vals = doubleArrayOf(40.0, 44.0, 60.0, 82.0, 100.0, 100.0, 100.0, 100.0)
