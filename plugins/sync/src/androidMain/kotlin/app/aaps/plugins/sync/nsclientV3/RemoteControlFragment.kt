...
import app.aaps.plugins.sync.nsShared.ModePresets
import app.aaps.plugins.sync.nsShared.ModesAdapter
import app.aaps.plugins.sync.nsShared.ModeCategory
/**
 ...
 * - Parent-friendly UI
 */
class RemoteControlFragment : Fragment(), PluginFragment {
    ...
    private var views: FragmentRemoteControlViews? = null
    ...
    override fun onCreateView(
        ...
        val views = FragmentRemoteControlViews.from(inflater.inflate(R.layout.fragment_remote_control, container, false))
        this.views = views
        return views.root
    }
    ...
    private fun showPinSetupDialog() {
        context?.let { ctx ->
            ...
            views?.pinSetupCard?.visibility = View.VISIBLE
        }
    }
    ...
    private fun setupPinInput() {
        ...
        if (pinConfigured) {
            views.pinInput.hint = rh.gs(R.string.enter_your_pin)
        }

        views.pinInput.addTextChangedListener {
            // PIN validation happens on mode send
        }

        // PIN Setup Card
        views.btnSavePin.setOnClickListener {
            val newPin = views.newPinInput.text.toString()
            val confirmPin = views.confirmPinInput.text.toString()
            ...
            when {
                ...
                newPin != confirmPin -> showError(rh.gs(R.string.pins_do_not_match))
                    ...
                    views.pinSetupCard.visibility = View.GONE
                    views.pinInput.hint = rh.gs(R.string.enter_your_pin)
                }
            }
        }
    ...
    private fun setupTabs() {
        ...
        // Setup ViewPager2
        views.viewPager.adapter = pagerAdapter

        // Link TabLayout with ViewPager2
        TabLayoutMediator(views.tabLayout, views.viewPager) { tab, position ->
    ...
    private fun sendModeCommand(mode: ModePreset, durationMin: Int) {
        val pin = views?.pinInput?.text?.toString() ?: ""
...
/**
 * The views of `fragment_remote_control.xml`, found by hand.
 *
 * A Kotlin Multiplatform module never gets generated view binding classes, so there is no
 * `FragmentRemoteControlBinding` to use. The names are the ones the generated class used.
 */
class FragmentRemoteControlViews(
    val root: LinearLayout,
    val pinSetupCard: MaterialCardView,
    val newPinInput: TextInputEditText,
    val confirmPinInput: TextInputEditText,
    val btnSavePin: Button,
    val pinInput: TextInputEditText,
    val tabLayout: TabLayout,
    val viewPager: ViewPager2,
) {
    companion object {
        fun from(root: View): FragmentRemoteControlViews = FragmentRemoteControlViews(
            root = root as LinearLayout,
            pinSetupCard = root.findViewById(R.id.pin_setup_card),
            newPinInput = root.findViewById(R.id.new_pin_input),
            confirmPinInput = root.findViewById(R.id.confirm_pin_input),
            btnSavePin = root.findViewById(R.id.btn_save_pin),
            pinInput = root.findViewById(R.id.pin_input),
