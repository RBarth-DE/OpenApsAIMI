...
class DataHandlerMobileWearBolusTest : TestBaseWithProfile() {
    ...
    @BeforeEach fun prepare() {
        sut = DataHandlerMobile(
            ...
            constraintsChecker, activePlugin, insulinManager, commandQueue, fabricPrivacy, uiInteraction,
            persistenceLayer, importExportPrefs, decimalFormatter, pumpStatusProvider,
            ch, runningModeGuard, wizardBolusExecutor, batchExecutor, wizardExecutor, uel
        )
     ...
     * subscribed before the block runs: RxBus has no replay, so a scheduled collector would miss
     * everything the block sends.
     */
    ...
    private inline fun captureAll(block: () -> Unit): List<EventData> {
        ...
        val job = collectMobileToWear { out += it }
        block()
        job.cancel()
        return out
    }
    ...
    private suspend fun negotiateRunningModes(allowed: List<RM.Mode>): EventData.RunningModeList {
        ...
        val job = collectMobileToWear { (it as? EventData.RunningModeList)?.let { l -> list = l } }
