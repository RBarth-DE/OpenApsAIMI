...
class NotificationStore @Inject constructor(
    ...
    inner class NotificationRecyclerViewAdapter internal constructor(private val notificationsList: List<Notification>) :
        ...
        override fun onBindViewHolder(holder: NotificationsViewHolder, position: Int) {
            ...
            else holder.dismiss.setText(app.aaps.core.ui.R.string.snooze)
            @Suppress("SetTextI18n")
            holder.text.text = dateUtil.timeString(notification.date) + " " + notification.text.orEmpty()
            when (notification.level) {
                Notification.URGENT       -> holder.cv.setBackgroundColor(rh.gac(app.aaps.core.ui.R.attr.notificationUrgent))
                Notification.NORMAL       -> holder.cv.setBackgroundColor(rh.gac(app.aaps.core.ui.R.attr.notificationNormal))
                Notification.LOW          -> holder.cv.setBackgroundColor(rh.gac(app.aaps.core.ui.R.attr.notificationLow))
                Notification.INFO         -> holder.cv.setBackgroundColor(rh.gac(app.aaps.core.ui.R.attr.notificationInfo))
                Notification.ANNOUNCEMENT -> holder.cv.setBackgroundColor(rh.gac(app.aaps.core.ui.R.attr.notificationAnnouncement))
            }
        }
        ...
        inner class NotificationsViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
            ...
            // binding classes, so there is no `OverviewNotificationItemBinding` to use.
            val cv: View = itemView
            val text: TextView = itemView.findViewById(R.id.text)
            val dismiss: MaterialButton = itemView.findViewById(R.id.dismiss)
