...
class OverviewFragment : Fragment(), View.OnClickListener, View.OnLongClickListener {
    ...
    // A Kotlin Multiplatform module never gets generated view binding classes, so the views of
    // overview_fragment.xml are found by hand in [OverviewFragmentViews].
    private var _binding: OverviewFragmentViews? = null
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View =
        inflater.inflate(R.layout.overview_fragment, container, false).also { root ->
            _binding = OverviewFragmentViews.from(root)
        }
    ...
    override fun onResume() {
        ...
        persistenceLayer.observeChanges(RM::class)
            .onEach { processAps() }
            .launchIn(newScope)
        // The bus subscriptions below replace the RxJava ones. The op that was `.observeOn(...)`
        // is now the dispatcher of the scope the collector runs in.
        activePlugin.activeOverview.overviewBus
            .toFlow(EventUpdateOverviewCalcProgress::class)
            .collectResilient(mainScope, aapsLogger, LTag.UI, start = CoroutineStart.UNDISPATCHED) { updateCalcProgress() }
        activePlugin.activeOverview.overviewBus
            .toFlow(EventUpdateOverviewIobCob::class)
            .debounce(1_000L)
            .collectResilient(newScope, aapsLogger, LTag.UI, start = CoroutineStart.UNDISPATCHED) { updateIobCob() }
        activePlugin.activeOverview.overviewBus
            .toFlow(EventUpdateOverviewSensitivity::class)
            .debounce(1_000L)
            .collectResilient(mainScope, aapsLogger, LTag.UI, start = CoroutineStart.UNDISPATCHED) { updateSensitivity() }
        activePlugin.activeOverview.overviewBus
            .toFlow(EventUpdateOverviewGraph::class)
            .debounce(1_000L)
            .collectResilient(mainScope, aapsLogger, LTag.UI, start = CoroutineStart.UNDISPATCHED) { updateGraph() }
        viewLifecycleOwner.lifecycleScope.launch {
            notificationManager.notifications.collectLatest { updateNotification() }
        }
        rxBus
            .toFlow(EventScale::class)
            .collectResilient(mainScope, aapsLogger, LTag.UI, start = CoroutineStart.UNDISPATCHED) { event ->
                forceGraphViewportReset = true
                overviewData.rangeToDisplay = event.hours
                preferences.put(IntNonKey.RangeToDisplay, event.hours)
                preferences.put(BooleanNonKey.ObjectivesScaleUsed, true)
            }
        rxBus
            .toFlow(EventBucketedDataCreated::class)
            .debounce(1_000L)
            .collectResilient(newScope, aapsLogger, LTag.UI, start = CoroutineStart.UNDISPATCHED) { updateBg() }
        rxBus
            .toFlow(EventRefreshOverview::class)
            .collectResilient(newScope, aapsLogger, LTag.UI, start = CoroutineStart.UNDISPATCHED) { event ->
                if (event.now) refreshAll()
                else scheduleUpdateGUI()
            }
        rxBus
            .toFlow(EventAcceptOpenLoopChange::class)
            .collectResilient(newScope, aapsLogger, LTag.UI, start = CoroutineStart.UNDISPATCHED) { scheduleUpdateGUI() }
        merge(
        ...
        rxBus
            .toFlow(EventNewOpenLoopNotification::class)
            .collectResilient(newScope, aapsLogger, LTag.UI, start = CoroutineStart.UNDISPATCHED) { scheduleUpdateGUI() }
        rxBus
            .toFlow(EventPumpStatusChanged::class)
            .collectResilient(mainScope, aapsLogger, LTag.UI, start = CoroutineStart.UNDISPATCHED) { event ->
                delay(30)
                overviewData.pumpStatus = rh.gs(event.getStatus())
                updatePumpStatus()
            }
        rxBus
            .toFlow(EventInitializationChanged::class)
            .collectResilient(mainScope, aapsLogger, LTag.UI, start = CoroutineStart.UNDISPATCHED) { processButtonsVisibility() }
                ...
                if (pump.pumpDescription.isTempBasalCapable) {
                    ...
                    when (loopMode) {
                        ...
                        RM.Mode.SUSPENDED_BY_PUMP -> {
                            ...
                            apsModeSetA11yLabel(app.aaps.core.interfaces.R.string.pumpsuspended)
                            binding.infoLayout.apsModeText.text = rh.gs(app.aaps.core.interfaces.R.string.pumpsuspended)
                            binding.infoLayout.apsModeText.visibility = View.GONE
                        }

                        RM.Mode.SUSPENDED_BY_USER -> {
                            binding.infoLayout.apsMode.setImageResource(app.aaps.core.ui.R.drawable.ic_loop_paused)
                            apsModeSetA11yLabel(app.aaps.core.interfaces.R.string.loopsuspended)
                            binding.infoLayout.apsModeText.text = dateUtil.age(minsToEndOfSuspend * 60000L, true, rh)
                            binding.infoLayout.apsModeText.visibility = View.VISIBLE
                        }
    ...
    private fun updateIobCob() {
        viewLifecycleOwner.lifecycleScope.launch {
            ...
            val iobText = rh.gs(app.aaps.core.interfaces.R.string.format_insulin_units, totalIob)
            val iobDialogText =
                rh.gs(app.aaps.core.interfaces.R.string.format_insulin_units, totalIob) + "\n" +
                    rh.gs(app.aaps.core.interfaces.R.string.bolus) + ": " + rh.gs(app.aaps.core.interfaces.R.string.format_insulin_units, bolusIob.iob) + "\n" +
                    rh.gs(app.aaps.core.ui.R.string.basal) + ": " + rh.gs(app.aaps.core.interfaces.R.string.format_insulin_units, basalIob.basaliob)
    ...
    private fun updateGraph() {
        _binding ?: return
        val pump = activePlugin.activePump
        val graphData = graphDataProvider().with(binding.graphsLayout.bgGraph, overviewData)
        ...
        for (g in 0 until min(secondaryGraphs.size, menuChartSettings.size - 1)) {
            val secondGraphData = graphDataProvider().with(secondaryGraphs[g], overviewData)
    ...
    private inner class NotificationRecyclerViewAdapter(
        ...
        override fun onBindViewHolder(holder: NotificationsViewHolder, position: Int) {
            ...
            val buttonText = notification.actions.firstOrNull()?.buttonText
            holder.binding.dismiss.text = if (buttonText != null) rh.gs(buttonText) else rh.gs(app.aaps.core.ui.R.string.snooze)
            ...
            when (notification.level) {
                ...
                NotificationLevel.IMPORTANT    -> holder.binding.cv.setBackgroundColor(rh.gac(app.aaps.core.ui.R.attr.notificationNormal))
        ...
        inner class NotificationsViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
            ...
            val binding = OverviewNotificationItemViews.from(itemView)
