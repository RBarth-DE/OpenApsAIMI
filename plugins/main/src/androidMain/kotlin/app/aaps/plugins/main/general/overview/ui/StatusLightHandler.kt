...
class StatusLightHandler @Inject constructor(
    ...
    private val concentrationHelper: ConcentrationHelper,
    ...
    fun updateStatusLights(
        ...
        if (pump.pumpDescription.isPatchPump) {
            handlePatchReservoirLevel(
                ...
                pump.reservoirLevel.value.iU(concentrationHelper.concentration),
                insulinUnit,
                pump.pumpDescription.maxReservoirReading.toDouble()
            )
        } else {
            handleLevel(reservoirLevel, IntKey.OverviewResCritical, IntKey.OverviewResWarning, pump.reservoirLevel.value.iU(concentrationHelper.concentration), insulinUnit)
        }
    ...
    /** "3 d 4 h" in short text, "3 days 4 hours" otherwise. The same text the dashboard shows. */
    private fun TE.age(useShortText: Boolean, rh: ResourceHelper, dateUtil: DateUtil): String {
        ...
        return if (useShortText)
            "${diff.days}${rh.gs(app.aaps.core.interfaces.R.string.shortday)}${diff.hours}${rh.gs(app.aaps.core.interfaces.R.string.shorthour)}"
        else
            "${diff.days} ${rh.gs(app.aaps.core.interfaces.R.string.days)} ${diff.hours} ${rh.gs(app.aaps.core.interfaces.R.string.hours)}"
    }
}
