...
internal class DashboardShellController(
    ...
    private val overviewDataCache get() = deps.overviewDataCache()
    private val persistenceLayer get() = deps.persistenceLayer

    /**
     * Runs the bus subscriptions. A RxBus subscription is a Flow now, so it lives in a scope rather
     * than in a RxJava disposable; [clearSubscriptions] cancels the collectors at the same points
     * the disposable used to be cleared.
     */
    private val subscriptionScope = CoroutineScope(Dispatchers.Main + SupervisorJob())
    ...
    fun resume() {
        ...
        rxBus.toFlow(EventPreferenceChange::class)
            .collectResilient(subscriptionScope, aapsLogger, LTag.UI, start = CoroutineStart.UNDISPATCHED) { event ->
    ...
    private fun updateGraph() {
        ...
        if (!useComposeOnlyGraphPipeline) {
            val graphView = binding.glucoseGraph ?: return
            val graphData = graphDataProvider().with(graphView.graph, overviewData)
