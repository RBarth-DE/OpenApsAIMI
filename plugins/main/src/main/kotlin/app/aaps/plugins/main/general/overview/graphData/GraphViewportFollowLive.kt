...
/**
 * True when the viewport's right edge still tracks [OverviewData.endTime] ("live").
 * Pass null for [viewportMaxMs] when the viewport has not been manually set (always live).
 */
fun viewportShouldFollowLiveRange(viewportMaxMs: Double?, overviewData: OverviewData): Boolean {
    if (viewportMaxMs == null || viewportMaxMs < 1_000_000_000_000.0) return true
    val liveEnd = overviewData.endTime.toDouble()
    val tolerance = liveEdgeToleranceMsForOverview(overviewData)
    return abs(viewportMaxMs - liveEnd) < tolerance
}
