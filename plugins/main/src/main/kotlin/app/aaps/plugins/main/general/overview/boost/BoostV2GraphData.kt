...
class BoostV2GraphData @Inject constructor(
    ...
    suspend fun addEps(context: Context?, scale: Double) {
    ...
    suspend fun addActivity(scale: Double) {
    ...
    suspend fun addMinusBGI(useForScale: Boolean, scale: Double) {
        if (useForScale) {
    ...
    suspend fun addAbsIob(useForScale: Boolean, scale: Double) {
        if (useForScale) {
    ...
    suspend fun addCob(useForScale: Boolean, scale: Double) {
        if (useForScale) {
    ...
    suspend fun addDeviations(useForScale: Boolean, scale: Double) {
        if (useForScale) {
    ...
    suspend fun addRatio(useForScale: Boolean, scale: Double) {
        if (useForScale) {
    ...
    suspend fun addDeviationSlope(useForScale: Boolean, scale: Double, isRatioScale: Boolean = false) {
        if (useForScale) {
    ...
    suspend fun addVarSens(useForScale: Boolean, scale: Double) {
        if (useForScale) {
    ...
    suspend fun addHeartRate(useForScale: Boolean, scale: Double) {
    ...
    suspend fun addSteps(useForScale: Boolean, scale: Double) {
     ...
     */
    suspend fun addInRangeArea(fromTime: Long, toTime: Long, lowLine: Double, highLine: Double) {
        // Filled band
     ...
     */
    private suspend fun addTargetDeviationZones() {
        ...
        suspend fun closeBand(endX: Double) {
            if (!bandOpen) return
            bandOpen = false
     ...
     */
    suspend fun addNowLine(now: Long) {
     ...
     */
    suspend fun addIob(useForScale: Boolean, scale: Double) {
        if (useForScale) {
            maxY = overviewData.maxIobValueFound
            minY = -overviewData.maxIobValueFound
        }
    ...
    private suspend fun addSeries(s: Series<*>) = series.add(s)
