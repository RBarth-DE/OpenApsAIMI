...
class GlucoseGraphView @JvmOverloads constructor(
    ...
    override fun performClick(): Boolean {
        // Calls the super code (important for accessibility)
        if (super.performClick()) return true

        // Here you can add additional logic
        // that should happen when the graph is clicked.
        return true
    }

    fun showPlaceholder(show: Boolean) {
        binding.graphPlaceholder.visibility = if (show) VISIBLE else GONE
        binding.graphView.visibility = if (show) GONE else VISIBLE
    }
}
