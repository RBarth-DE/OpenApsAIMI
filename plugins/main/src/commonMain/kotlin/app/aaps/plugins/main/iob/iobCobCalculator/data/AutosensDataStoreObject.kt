...
class AutosensDataStoreObject : AutosensDataStore {
    ...
    // earlier merge replaced this body with the upstream one and left the age check used only by the
    // dead copy above; the test `getLastAutosensDataTest` pins it, and it is restored here.
    override fun getLastAutosensData(reason: String, aapsLogger: AAPSLogger, dateUtil: DateUtil): AutosensData? {
        lock.lock()
        try {
            if (autosensDataTable.size() < 1) {
