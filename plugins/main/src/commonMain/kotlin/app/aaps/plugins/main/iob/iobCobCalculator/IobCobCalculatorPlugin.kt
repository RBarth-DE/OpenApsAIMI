...
class IobCobCalculatorPlugin(
    ...
    override suspend fun onStart() {
        ...
        // EventAppInitialized fires once, early. UNDISPATCHED matters here: a scheduled collector
        // could miss it outright and the deferred main calculation would never be kicked off.
            ...
            scheduledData?.let {
                ...
                aapsLogger.debug(
                    ...
                    "scheduleHistoryDataChange merged into pending ts=${dateUtil.dateAndTimeAndSecondsString(it.oldDataTimestamp)} " +
                        "reload=${it.reloadBgData} bgTrigger=${it.triggeredByNewBG}"
                )
            }
        }
    }
    ...
    private suspend fun newHistoryData(oldDataTimestamp: Long, bgDataReload: Boolean, triggeredByNewBG: Boolean) {
        ...
        calculationWorkflow.runCalculation(
            ...
            end = dateUtil.now(),            bgDataReload = bgDataReload,
