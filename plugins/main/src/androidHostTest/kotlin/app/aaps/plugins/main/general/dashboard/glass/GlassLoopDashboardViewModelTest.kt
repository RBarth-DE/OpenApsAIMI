...
class GlassLoopDashboardViewModelTest {
    ...
    fun setup() {
        ...
        // The view model reads two kinds of string: the shared ones as TextRef and its own as Int.
        // `anyVararg()` matches the whole vararg array whatever its length, so one stub per overload
        // covers every call - a stub written as `gs(any(), any())` registers two matchers for one
        // array argument and then never matches.
        whenever(resourceHelper.gs(any<TextRef>(), anyVararg())).thenReturn("formatted")
        whenever(resourceHelper.gs(any<Int>(), anyVararg())).thenReturn("formatted")
        // The no-argument calls ("never trained", "file missing", the mmol/mgdl unit label) need their
        // own stubs: a call with no args is a different method, not the same one with an empty array.
        whenever(resourceHelper.gs(any<TextRef>())).thenReturn("unit")
        whenever(resourceHelper.gs(any<Int>())).thenReturn("no value")
