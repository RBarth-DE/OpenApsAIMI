...
class OpenAPSAutoISFPluginTest : TestBaseWithProfile() {
    ...
    // The fork's plugin carries the MTR providers and a Context on top of the upstream parameters.
    // None of the tests below reads them, so plain mocks are enough.
    @Mock lateinit var automationStateService: AutomationStateInterface
    @Mock lateinit var unifiedActivityProvider: UnifiedActivityProviderMTR
    @Mock lateinit var physioAdapter: AIMIInsulinDecisionAdapterMTR
    @Mock lateinit var stepsManager: AIMIStepsManagerMTR
    ...
    @BeforeEach fun prepare() {
        openAPSAutoISFPlugin = OpenAPSAutoISFPlugin(
            ...
            // The fork's calculator has no DecimalFormatter: the only line that used it is commented out.
            GlucoseStatusCalculatorAutoIsf(aapsLogger, iobCobCalculator, dateUtil, deltaCalculator), { apsResultProvider() }, ch,
            automationStateService, unifiedActivityProvider, physioAdapter, stepsManager, { loop }, context
        )
    }
    ...
    /**
     * The fork's `withinISFlimits` takes the exercise / resistance / step flags instead of the
     * upstream `origin_sens`, `temptargetSet`, `target_bg` and `normalTarget` values, so the calls
     * here differ from upstream's test of the same name. The numbers are the same though: the factor
     * is clamped first, and only then does the autosens ratio and the mode come in.
     */
    ...
    fun withinISFLimitsTest() {
        ...
        // The clamp runs for every branch: 1.7 is capped at autoISF_max, 0.5 is raised to autoISF_min.
        assertThat(openAPSAutoISFPlugin.withinISFlimits(1.7, autoIsfMin, autoIsfMax, 1.1, false, false, false, false)).isEqualTo(1.2) // upper limit
        assertThat(openAPSAutoISFPlugin.withinISFlimits(0.5, autoIsfMin, autoIsfMax, 1.1, false, false, false, false)).isEqualTo(0.7) // lower limit
        // No mode running: a clamped factor of 1 or more keeps the higher of the two, below 1 the lower one.
        assertThat(openAPSAutoISFPlugin.withinISFlimits(1.7, autoIsfMin, autoIsfMax, 1.5, false, false, false, false)).isEqualTo(1.5) // autosens 1.5 wins
        assertThat(openAPSAutoISFPlugin.withinISFlimits(0.5, autoIsfMin, autoIsfMax, 0.5, false, false, false, false)).isEqualTo(0.5) // autosens 0.5 wins
        // A running mode multiplies the clamped factor by the autosens ratio: 0.7 * 0.5.
        assertThat(openAPSAutoISFPlugin.withinISFlimits(0.5, autoIsfMin, autoIsfMax, 0.5, true, false, false, false)).isEqualTo(0.35) // exercise mode
    }
