...
class KalmanFilterTest {
    ...
    /**
     * A TDD calculator that answers the same amount on the background refresh and on the
     * preference fallback.
     *
     * `KalmanISFCalculator` warms its TDD cache from `Dispatchers.IO`, and it reads the preference
     * until that refresh lands. A relaxed mock would answer 0.0 there, so the calculator would see
     * an unusable TDD on a fast machine and the real one on a slow machine - the same test would
     * pass or fail with the speed of the host.
     */
    private fun tddCalculatorReporting(amount: Double): TddCalculator {
        val tddCalculator = mockk<TddCalculator>(relaxed = true)
        every { tddCalculator.averageTDD(any()) } returns AverageTDD(
            data = TDD(timestamp = 0L, totalAmount = amount),
            allDaysHaveCarbs = true,
        )
    ...
    fun `test KalmanISFCalculator`() {
        val tddCalculator = tddCalculatorReporting(50.0)
        val preferences = mockk<Preferences>(relaxed = true)
        val logger = mockk<AAPSLogger>(relaxed = true)

        // The calculator and the preference answer the same amount, so the test does not depend on
        // which of the two was read first. See tddCalculatorReporting.
    ...
    fun `ISF at high glucose stays above the physiological floor instead of saturating on 5`() {
        val tddCalculator = tddCalculatorReporting(55.0)
        val preferences = mockk<Preferences>(relaxed = true)
        val logger = mockk<AAPSLogger>(relaxed = true)
    ...
    fun `an unusable TDD falls back to the absolute floor`() {
        val tddCalculator = tddCalculatorReporting(0.0)
        val preferences = mockk<Preferences>(relaxed = true)
        val logger = mockk<AAPSLogger>(relaxed = true)
