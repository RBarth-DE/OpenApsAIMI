...
class DetermineBasalAutoISFTest : TestBaseWithProfile() {
    ...
    @Mock private lateinit var pumpCapabilityValidator: PumpCapabilityValidator

    /**
     * The SOS check always runs, and it writes its state to a preferences file before it returns.
     * `RETURNS_SELF` is what makes the chained `putLong(...).putBoolean(...)` inside one `edit {}`
     * work on a mock.
     */
    private val sosPreferences: SharedPreferences = mock {
        on { edit() } doReturn mock<SharedPreferences.Editor>(defaultAnswer = Answers.RETURNS_SELF)
    }
    ...
    @BeforeEach
    fun setup() {
        // Every determine_basal run ends in the fork's SOS check. It stays off here, so it returns
        // before any SMS is sent, but it still reads the phone numbers and its saved state first.
        // A bare mock answers null for all three, and a null phone number crashes the trim() below.
        whenever(preferences.get(BooleanKey.AimiEmergencySosEnable)).thenReturn(false)
        whenever(preferences.get(StringKey.AimiEmergencySosPhone)).thenReturn("")
        whenever(preferences.get(StringKey.AimiEmergencySosPhone2)).thenReturn("")
        whenever(context.applicationContext).thenReturn(context)
        whenever(context.getSharedPreferences(anyString(), anyInt())).thenReturn(sosPreferences)

        sut = DetermineBasalAutoISF(profileUtil, context)
        // The injector fills these in the app. A hand-built instance starts with them unset, and the
        // first read would throw UninitializedPropertyAccessException.
        sut.aapsLogger = aapsLogger
        sut.activePlugin = activePlugin
        sut.preferences = preferences
        sut.pumpCapabilityValidator = pumpCapabilityValidator
    }
