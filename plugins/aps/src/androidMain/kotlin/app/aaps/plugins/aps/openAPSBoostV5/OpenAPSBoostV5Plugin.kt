...
open class OpenAPSBoostV5Plugin @Inject constructor(
    ...
    // Provider breaks the DI cycle (OpenAPSBoostPlugin injects () -> OpenAPSBoostV5Plugin for runShadow).
    private val openAPSBoostEngine: () -> OpenAPSBoostPlugin,
    ...
    // @SingleIn(AppScope::class) — same instance the V1 engine scored this cycle; its cached feature vector powers
    ...
    PluginDescription()
        ...
        .pluginName(ApsStrings.openaps_boost_v6)
        .shortName(ApsStrings.boost_v6_shortname)
        .preferencesVisibleInSimpleMode(false)
        .showInList { config.APS }
        .description(ApsStrings.description_boost_v6)
    ...
    override fun getPreferenceScreenContent() = app.aaps.core.ui.compose.preference.PreferenceSubScreenDef(
        ...
        items = listOf(
            ...
            BooleanKey.ApsBoostV5PrimerBolusMode,
            ...
            app.aaps.core.ui.compose.preference.PreferenceSubScreenDef(
                ...
                titleResId = app.aaps.core.ui.R.string.openaps_boost_v1_settings_title,
                items = listOf(
    ...
    override var lastAPSResult: APSResult?
        get() = openAPSBoostEngine().lastAPSResult
        set(_) {}
    override var lastAPSRun: Long
        get() = openAPSBoostEngine().lastAPSRun
        set(_) {}
    ...
    override suspend fun invoke(initiator: String, tempBasalFallback: Boolean) {
        ...
        openAPSBoostEngine().runEngine(initiator, tempBasalFallback, v5Active = true)
    }
    ...
    internal suspend fun composedFloorTbrAllowed(now: Long): Boolean {
    ...
    private suspend fun maybeAutoConfigure() {
        ...
        if (applied.isNotEmpty() || heldSuggestions.isNotEmpty()) {
            ...
            aapsLogger.info(LTag.APS, "Boost V6 set ${applied.size} setting(s) from your last 14 days (your other settings were kept):\n$pretty")
        }
    }
    ...
    private suspend fun buildInputs(
        rT: RT,
        gs: GlucoseStatus,
        iobArray: Array<IobTotal>,
    ...
    /** V5 headline tuning knobs — read via [getBoostDosing] to bypass Simple Mode masking. */
    ...
    override fun getGlucoseStatusData(allowOldData: Boolean): GlucoseStatus? =
        openAPSBoostEngine().getGlucoseStatusData(allowOldData)

    override fun supportsDynamicIc(): Boolean = openAPSBoostEngine().supportsDynamicIc()
    override fun getIc(profile: Profile): Double? = openAPSBoostEngine().getIc(profile)
    override fun getIc(timestamp: Long, profile: Profile): Double? = openAPSBoostEngine().getIc(timestamp, profile)
    override fun getIsfMgdl(profile: Profile, caller: String): Double? = openAPSBoostEngine().getIsfMgdl(profile, caller)
    override fun getAverageIsfMgdl(timestamp: Long, caller: String): Double? = openAPSBoostEngine().getAverageIsfMgdl(timestamp, caller)
    override fun getSensitivityOverviewString(): String? = openAPSBoostEngine().getSensitivityOverviewString()
    ...
    override suspend fun isSMBModeEnabled(value: Constraint<Boolean>): Constraint<Boolean> = openAPSBoostEngine().isSMBModeEnabled(value)
    override fun isUAMEnabled(value: Constraint<Boolean>): Constraint<Boolean> = openAPSBoostEngine().isUAMEnabled(value)
    override fun isAutosensModeEnabled(value: Constraint<Boolean>): Constraint<Boolean> = openAPSBoostEngine().isAutosensModeEnabled(value)
    override fun isSuperBolusEnabled(value: Constraint<Boolean>): Constraint<Boolean> = openAPSBoostEngine().isSuperBolusEnabled(value)
    override suspend fun applyMaxIOBConstraints(maxIob: Constraint<Double>): Constraint<Double> = openAPSBoostEngine().applyMaxIOBConstraints(maxIob)
    override fun applyBasalConstraints(absoluteRate: Constraint<Double>, profile: Profile): Constraint<Double> = openAPSBoostEngine().applyBasalConstraints(absoluteRate, profile)
    ...
    override suspend fun onStart() { super.onStart(); openAPSBoostEngine().onStart() }
    override suspend fun onStop() { openAPSBoostEngine().onStop(); super.onStop() }

    /* override */ fun configuration(): JSONObject = openAPSBoostEngine().configuration()

    /* override */ fun applyConfiguration(configuration: JSONObject) = openAPSBoostEngine().applyConfiguration(configuration)
