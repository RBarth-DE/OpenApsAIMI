...
private fun formatPkpdRecommendationDescription(recommendation: AimiRecommendation): String {
    ...
    // AimiRecommendation still carries Android ids, so this one is bridged. It renders as "?"
    // off Android, which is the marker for a caller left to convert.
    val description = TextRef.AndroidRes(recommendation.descriptionResId)
    return when (args.size) {
        0 -> stringResource(description)
        1 -> stringResource(description, args[0])
        2 -> stringResource(description, args[0], args[1])
        else -> stringResource(description)
    }
}
...
fun PkpdAdvisorSuggestionCard(
    ...
    Card(
        ...
        Column(Modifier.padding(AapsSpacing.medium)) {
            Text(stringResource(TextRef.AndroidRes(recommendation.titleResId)), style = MaterialTheme.typography.titleSmall)
            Text(description, style = MaterialTheme.typography.bodySmall, modifier = Modifier.padding(top = AapsSpacing.extraSmall))
            Button(
                onClick = { onApply(action) },
                modifier = Modifier.padding(top = AapsSpacing.small),
            ) {
                Text(stringResource(ApsStrings.aimi_pkpd_advisor_apply))
            }
        }
    }
}
...
fun PkpdReactiveDoubleSlider(
    ...
    val summary = key.summary?.let { stringResource(it) }
    ...
    Column(
        ...
        SliderWithButtons(
            ...
            step = step,
            showValue = true,
            unitFormat = unitType.unitFormat(),
