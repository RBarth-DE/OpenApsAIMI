...
class   DetermineBasalBoost @Inject constructor(
    ...
    fun determine_basal(
        ...
        // never computed.
        // Create RT via property setters to avoid the 137-param constructor call
        // that triggers a d8/Kotlin verifier bug with aget-object on String.
        var rT = RT()
        rT.algorithm = APSResult.Algorithm.BOOST
        rT.runningDynamicIsf = true
        rT.timestamp = currentTime
        rT.consoleLog = consoleLog
        rT.consoleError = consoleError
        rT.boostActive = profile.boostActive
        rT.boostProfileSwitch = profile.profileSwitch
        rT.boostTier = "NONE"
        rT.fastCarbProtection = false
        rT.insulinReqPctEffective = 0.0
        rT.sensNormalTarget = if (profile.sensNormalTarget > 0) round(profile.sensNormalTarget, 1) else null
        rT.predictionISF = if (profile.variable_sens > 0) round(profile.variable_sens, 1) else null
        rT.tdd = if (profile.TDD > 0) round(profile.TDD, 1) else null
        ...
        val stateIob = iob_data_array[0].iob
        val stateActivity = iob_data_array[0].activity
        consoleError.add("IOB: ${round(stateIob, 2)} | Activity: ${round(stateActivity, 4)} | COB: ${meal_data.mealCOB}")
        ...
        // =====================================================================
        rT = RT()
        rT.algorithm = APSResult.Algorithm.SMB
        rT.runningDynamicIsf = true
        rT.timestamp = currentTime
        rT.bg = bg
        rT.tick = tick
        ...
        rT.consoleLog = consoleLog
        rT.consoleError = consoleError
        rT.variable_sens = sens
        // Boost/DynISF fields for Nightscout upload
        rT.boostActive = profile.boostActive
        rT.predictionISF = round(profile.variable_sens, 1)
        rT.sensNormalTarget = round(profile.sensNormalTarget, 1)
        rT.tdd = if (profile.TDD > 0) round(profile.TDD, 1) else null
        rT.tddRatio = sensitivityRatio
        rT.deltaAcceleration = delta_accl
        rT.boostProfileSwitch = profile.profileSwitch
        rT.boostTier = "NONE"
        rT.fastCarbProtection = false
        rT.insulinReqPctEffective = 0.0
