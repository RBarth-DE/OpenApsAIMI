        ...
        val reason = when (preferredAlgorithm) {
            "AIMI" -> {
                if (aggressivenessRatio > 2.0) {
                    "SMB ${String.format("%.1f", aggressivenessRatio)}x plus aggressive with variability ${safety.variabilityLabel.lowercase()}"
                } else {
            ...
            else -> "Both algorithms show similar performance. (${String.format("%.1f", stats.agreementRate)}% OK)"
        }
