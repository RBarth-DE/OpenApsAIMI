...
class AutoIsfAiClient {
    ...
    private fun buildPrompt(report: AutoIsfReport, prefs: AutoIsfPrefsSnapshot, context: Context): String {
        ...
        sb.appendLine("  highBgWeight=${prefs.highBgWeight}         (min: ${DoubleKey.ApsAutoIsfHighBgWeight.min} / max: ${DoubleKey.ApsAutoIsfHighBgWeight.max} / disabled: 0)   (boosts ISF at high BG)")
        sb.appendLine("  lowBgWeight=${prefs.lowBgWeight}           (min: ${DoubleKey.ApsAutoIsfLowBgWeight.min} / max: ${DoubleKey.ApsAutoIsfLowBgWeight.max} / disabled: 0)     (lowers ISF when BG low → more insulin)")
        sb.appendLine("  bgAccelWeight=${prefs.bgAccelWeight}       (min: ${DoubleKey.ApsAutoIsfBgAccelWeight.min} / max: ${DoubleKey.ApsAutoIsfBgAccelWeight.max} / disabled: 0) (responds to rising BG rate)")
        sb.appendLine("  bgBrakeWeight=${prefs.bgBrakeWeight}       (min: ${DoubleKey.ApsAutoIsfBgBrakeWeight.min} / max: ${DoubleKey.ApsAutoIsfBgBrakeWeight.max} / disabled: 0) (reduces ISF when BG falling fast)")
        sb.appendLine("  ppWeight=${prefs.ppWeight}                 (min: ${DoubleKey.ApsAutoIsfPpWeight.min} / max: ${DoubleKey.ApsAutoIsfPpWeight.max} / disabled: 0)           (post-prandial ISF boost)")
        sb.appendLine("  duraWeight=${prefs.duraWeight}             (min: ${DoubleKey.ApsAutoIsfDuraWeight.min} / max: ${DoubleKey.ApsAutoIsfDuraWeight.max} / disabled: 0)       (extra ISF for prolonged highs)")
