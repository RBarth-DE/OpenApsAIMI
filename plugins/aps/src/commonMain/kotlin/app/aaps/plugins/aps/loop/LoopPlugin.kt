...
import app.aaps.plugins.aps.loop.compose.LoopComposeContent
...
class LoopPlugin(
    ...
    private val invokeCoalescingLock = AapsLock()
    ...
    // The periodic autodrive ticker. A coroutine rather than a Handler: `android.os.Handler` is not in
    // shared code, and this was only ever a timer - the tick body already does its work in appScope.
    ...
    private fun elapsedRealtimeMs(): Long = monotonicStart.elapsedNow().inWholeMilliseconds
    ...
    private fun startPeriodicLoop() {
        // Cancel first: a second onStart() without an onStop() would otherwise leave two tickers
        // running, and each one calls invoke() on its own period.
        periodicLoopJob?.cancel()
        val initialFreqMs = T.mins(preferences.get(IntKey.ApsMaxSmbFrequency).toLong()).msecs()
        periodicLoopJob = appScope.launch {
            delay(initialFreqMs)
            while (isActive) {
    ...
    private suspend fun reconcileRunningMode() {
        ...
        while (true) {
            ...
            if (config.APS) {
                val pump = activePlugin.activePump
                // Reconcile only against a pump that is really reachable. While it is connecting or
                // handshaking, a pump reports suspend states the user never asked for, and isSuspended()
                // is wide: Medtrum counts every state below ACTIVE as suspended.
                val mayReconcilePumpRunningMode = pump.isInitialized() &&
                    pump.isConnected() &&
                    !pump.isConnecting() &&
                    !pump.isHandshakeInProgress()
                // isSuspendedForRunningModeReconciliation() is the pump's own answer to "is the user really
                // suspended" (`MedtrumPlugin`); the short-circuit keeps the question from reaching a pump
                // that is not ready to answer it.
                val pumpSuspended = mayReconcilePumpRunningMode && pump.isSuspendedForRunningModeReconciliation()
                ...
                if (mayReconcilePumpRunningMode && !pumpSuspended && runningMode.mode == RM.Mode.SUSPENDED_BY_PUMP) {
    ...
    override suspend fun invoke(initiator: String, allowNotification: Boolean, tempBasalFallback: Boolean) {
        val incoming = LoopInvokeRequest(
            ...
            requestedAtElapsedMs = elapsedRealtimeMs()
        )

        val shouldDrain = invokeCoalescingLock.withLock {
        ...
        while (true) {
            val next = invokeCoalescingLock.withLock {
            ...
            val waitStartedAt = elapsedRealtimeMs()
            try {
                invokeMutex.withLock {
                    val waitedMs = elapsedRealtimeMs() - waitStartedAt
                    if (waitedMs >= 250L) {
                        aapsLogger.warn(
                            LTag.APS,
                    ...
                    val queuedForMs = elapsedRealtimeMs() - next.requestedAtElapsedMs
                    if (queuedForMs >= 500L) {
                        aapsLogger.warn(
                            LTag.APS,
                ...
                if (runningMode().isClosedLoopOrLgs()) {
                    ...
                    if (resultAfterConstraints.isChangeRequested()
                            ...
                            if (tbrResult.enacted || tbrResult.success) {
                                ...
                                if (resultAfterConstraints.isBolusRequested) {
                                    ...
                                    aapsLogger.debug(
                                        LTag.APS,
                                        formatTemplate(
                                            "SMB enact result: requested=%.2fU enacted=%s success=%s comment=%s",
                                            listOf(
                                ...
                                aapsLogger.debug(LTag.APS, formatTemplate("SMB skipped: TBR not enacted/success (smb=%.2fU comment=%s) — carry-forward will retry", listOf(resultAfterConstraints.smb, tbrResult.comment ?: "")))
                            }
                        }
                        rxBus.send(EventLoopUpdateGui())
                    } else {
                        ...
                        if (resultAfterConstraints.isBolusRequested && commandQueue.bolusInQueue())
                            aapsLogger.debug(LTag.APS, formatTemplate("SMB skipped: bolus already in queue (smb=%.2fU) — carry-forward will retry", listOf(resultAfterConstraints.smb)))
                        lastRun.tbrSetByPump = null
                        lastRun.smbSetByPump = null
                    }
                } else {
    ...
    private suspend fun applySMBRequest(request: APSResult): PumpEnactResult {
        ...
        if (lastBolusTime != 0L && lastBolusTime + T.mins(smbIntervalMin.toLong()).msecs() > now) {
            aapsLogger.debug(
                LTag.APS,
                formatTemplate(
                    "SMB blocked by interval: requested=%.2fU lastBolusAge=%.0fs interval=%dm deliverAt=%s",
                    listOf(
                        request.smb,
                        lastBolusAgeSec,
                        smbIntervalMin,
            ...
            return pumpEnactResultProvider()
                .comment(ApsStrings.smb_frequency_exceeded)
        ...
        aapsLogger.debug(
            LTag.APS,
            formatTemplate(
                "applySMBRequest: requested=%.2fU deliverAt=%s lastBolusAge=%.0fs pumpStep=%.3fU",
                listOf(
                    request.smb,
                    request.deliverAt?.let { dateUtil.dateAndTimeAndSecondsString(it) } ?: "n/a",
                    lastBolusAgeSec,
        ...
        if (config.APS) {
            ...
            if (!tbrResult.success) {
                uiInteraction.runAlarm(
                    tbrResult.comment,
                    rh.gs(CoreUiStrings.temp_basal_delivery_error),
            ...
            if (pump.pumpDescription.isExtendedBolusCapable && persistenceLayer.getExtendedBolusActiveAt(dateUtil.now()) != null) {
                ...
                if (!cancelEbResult.success) {
                    uiInteraction.runAlarm(
                        ...
                        rh.gs(CoreUiStrings.extendedbolusdeliveryerror),
                        AlarmSound.BOLUS_ERROR,
                    )
                }
            }
