...
class OpenAPSAMAPlugin(
    ...
    override suspend fun invoke(initiator: String, tempBasalFallback: Boolean) = withContext(Dispatchers.Default) {
        ...
        if (!hardLimits.checkHardLimits(profile.getIsfMgdl("OpenAPSAMAPlugin"), InterfacesStrings.profile_sensitivity_value, HardLimits.LIMIT_ISF)) return@withContext
    ...
    override suspend fun applyMaxIOBConstraints(maxIob: Constraint<Double>): Constraint<Double> {
        if (isEnabled()) {
            ...
            maxIob.setIfSmaller(hardLimits.maxIobAMA(), rh.gs(ApsStrings.limiting_iob, hardLimits.maxIobAMA(), rh.gs(ApsStrings.hardlimit)), this)
        }
        return maxIob
    }
