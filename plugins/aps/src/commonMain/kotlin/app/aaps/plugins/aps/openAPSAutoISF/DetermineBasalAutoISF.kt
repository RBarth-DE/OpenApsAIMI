...
class DetermineBasalAutoISF(
    ...
    // The SOS check below goes through the AIMI EmergencySosManager, which needs a Context to send
    // the SMS. Same shape as DetermineBasalAIMI2.
    private val context: Context,
) {

    @Inject lateinit var aapsLogger: AAPSLogger
    @Inject lateinit var activePlugin: ActivePlugin
    // Read by the SOS check: the phone numbers and the on/off switches live in preferences.
    @Inject lateinit var preferences: Preferences
    // Keeps a proposed basal inside what the pump can actually do (max basal, basal step).
    @Inject lateinit var pumpCapabilityValidator: PumpCapabilityValidator
