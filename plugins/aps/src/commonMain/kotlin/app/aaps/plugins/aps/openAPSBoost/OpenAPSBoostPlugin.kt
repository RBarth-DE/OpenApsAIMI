...
open class OpenAPSBoostPlugin @Inject constructor(
    ...
    // This plugin still needs Android resource ids (R.string literals), so it keeps the Android
    // resolver instead of the TextResolver the migrated plugins use.
    ...
    // only way V5 sees a cycle's data. The lambda avoids a hard DI cycle.
    private val boostV5Plugin: () -> app.aaps.plugins.aps.openAPSBoostV5.OpenAPSBoostV5Plugin,
    ...
    override suspend fun onStart() {
        super.onStart()
        // EventCalibrationDetected subscription removed — v4 uses different calibration event model
    }
    ...
    private fun engineActive(): Boolean =
        isEnabled() || runCatching { boostV5Plugin().isEnabled() }.getOrDefault(false)
    ...
    private suspend fun calculateBoostActivity(
        now: Long,
     ...
     */
    suspend fun runEngine(initiator: String, tempBasalFallback: Boolean, v5Active: Boolean) {
        ...
        if (!hardLimits.checkHardLimits(profile.iCfg.dia, app.aaps.core.interfaces.R.string.profile_dia, hardLimits.minDia(), hardLimits.maxDia())) return
        ...
        if (!hardLimits.checkHardLimits(profile.getIsfMgdl("OpenAPSBoostPlugin"), app.aaps.core.interfaces.R.string.profile_sensitivity_value, HardLimits.MIN_ISF, HardLimits.MAX_ISF)) return
        ...
        val insulinPeak = profile.iCfg.peak.coerceIn(30, 75)
        ...
        val currentBasal = if (activityResult.profileSwitch != 100) {
            val adjusted = pump.baseBasalRate.cU * profileScale
            aapsLogger.debug(LTag.APS, "Basal adjusted to $adjusted")
            adjusted
        } else pump.baseBasalRate.cU
        ...
        if (recentLowBG < 100.0 && glucoseStatus.glucose > recentLowBG + 20) {
            ...
            if (activeTt != null && activeTt.reason == TT.Reason.ACTIVITY) {
                ...
                isTempTarget = false
                minBg = hardLimits.verifyHardLimits(Round.roundTo(profile.getTargetLowMgdl(), 0.1), app.aaps.core.interfaces.R.string.profile_low_target, HardLimits.LIMIT_MIN_BG)
                maxBg = hardLimits.verifyHardLimits(Round.roundTo(profile.getTargetHighMgdl(), 0.1), app.aaps.core.interfaces.R.string.profile_high_target, HardLimits.LIMIT_MAX_BG)
                targetBg = hardLimits.verifyHardLimits(profile.getTargetMgdl(), app.aaps.core.ui.R.string.temp_target_value, HardLimits.LIMIT_TARGET_BG)
            }
        }
        ...
        determineBasalBoost.determine_basal(
            ...
            try {
                ...
                if (darkEvent.raiseNotification) {
                    aapsLogger.warn(LTag.APS, "Boost: heart-rate feed has been dark for ${darkEvent.darkMinutes} min — check the watch/Garmin connection")
                }
    ...
    private suspend fun isNightModeActive(): Boolean {
    ...
    private suspend fun isNightModeActiveImpl(): Boolean {
        if (!preferences.getBoostDosing(BooleanKey.ApsBoostNightModeEnabled)) return false
    ...
    fun addPreferenceScreen(preferenceManager: PreferenceManager, parent: PreferenceScreen, context: Context, requiredKey: String?) {
        return // v3 XML preferences removed for v4
    }
}
