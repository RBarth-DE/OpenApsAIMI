...
internal data class AimiBehaviorFamilySnapshot(
    ...
    val title: TextRef,
    val question: TextRef,
    val leftAnchor: TextRef,
    val rightAnchor: TextRef,
    val levelLabel: TextRef,
...
internal enum class AimiProjectionStatus(val label: TextRef) {
...
internal enum class AimiT3cRuntimeStatus(val label: TextRef) {
...
internal enum class AimiT3cRuntimeOwner(val label: TextRef) {
...
internal enum class AimiHarmoniaRuntimeStatus(val label: TextRef) {
...
private fun buildProtectionFamily(preferences: Preferences): AimiBehaviorFamilySnapshot {
    ...
    return AimiBehaviorFamilySnapshot(
        ...
        title = ApsStrings.aimi_control_center_protection_title,
        question = ApsStrings.aimi_control_center_protection_question,
        leftAnchor = ApsStrings.aimi_control_center_protection_left,
        rightAnchor = ApsStrings.aimi_control_center_protection_right,
        levelLabel = protectionLevelLabel(projection.score),
...
private fun buildMealCaptureFamily(preferences: Preferences): AimiBehaviorFamilySnapshot {
    ...
    return AimiBehaviorFamilySnapshot(
        ...
        title = ApsStrings.aimi_control_center_meal_title,
        question = ApsStrings.aimi_control_center_meal_question,
        leftAnchor = ApsStrings.aimi_control_center_meal_left,
        rightAnchor = ApsStrings.aimi_control_center_meal_right,
        levelLabel = mealLevelLabel(projection.score),
        normalizedScore = projection.score,
        confidence = projection.confidence,
...
private fun buildStabilityFamily(
    ...
    return AimiBehaviorFamilySnapshot(
        ...
        title = ApsStrings.aimi_control_center_stability_title,
        question = ApsStrings.aimi_control_center_stability_question,
        leftAnchor = ApsStrings.aimi_control_center_stability_left,
        rightAnchor = ApsStrings.aimi_control_center_stability_right,
        levelLabel = stabilityLevelLabel(projection.score),
        normalizedScore = projection.score,
        confidence = projection.confidence,
...
private fun buildPhysioFamily(
    ...
    return AimiBehaviorFamilySnapshot(
        ...
        title = ApsStrings.aimi_control_center_physio_title,
        question = ApsStrings.aimi_control_center_physio_question,
        leftAnchor = ApsStrings.aimi_control_center_physio_left,
        rightAnchor = ApsStrings.aimi_control_center_physio_right,
        levelLabel = physioLevelLabel(projection.score),
        normalizedScore = projection.score,
        confidence = projection.confidence,
...
private fun buildAutonomyFamily(preferences: Preferences): AimiBehaviorFamilySnapshot {
    ...
    return AimiBehaviorFamilySnapshot(
        ...
        title = ApsStrings.aimi_control_center_autonomy_title,
        question = ApsStrings.aimi_control_center_autonomy_question,
        leftAnchor = ApsStrings.aimi_control_center_autonomy_left,
        rightAnchor = ApsStrings.aimi_control_center_autonomy_right,
...
private fun buildContextSection(preferences: Preferences): AimiControlSectionSnapshot =
    AimiControlSectionSnapshot(
        title = ApsStrings.aimi_control_center_context_title,
        summary = ApsStrings.aimi_control_center_context_summary,
        details = listOf(
            ...
            AimiControlDetail(
                title = ApsStrings.OApsAIMI_Enable_pregnancy,
                value = if (preferences.get(BooleanKey.OApsAIMIpregnancy)) CoreUiStrings.yes else CoreUiStrings.no,
            ),
            AimiControlDetail(
                title = ApsStrings.OApsAIMI_Enable_honeymoon,
                value = if (preferences.get(BooleanKey.OApsAIMIhoneymoon)) CoreUiStrings.yes else CoreUiStrings.no,
            ),
            AimiControlDetail(
                title = ApsStrings.aimi_control_center_cycle_module_title,
                value = if (preferences.get(BooleanKey.OApsAIMIwcycle)) CoreUiStrings.yes else CoreUiStrings.no,
            ),
            AimiControlDetail(
                title = ApsStrings.oaps_aimi_thyroid_enabled_title,
                value = if (preferences.get(BooleanKey.OApsAIMIThyroidEnabled)) CoreUiStrings.yes else CoreUiStrings.no,
            ),
            AimiControlDetail(
                title = ApsStrings.endo_enable_title,
                value = if (preferences.get(BooleanKey.AimiEndometriosisEnable)) CoreUiStrings.yes else CoreUiStrings.no,
            ),
            AimiControlDetail(
                title = ApsStrings.oaps_aimi_ngr_enabled_title,
                value = if (preferences.get(BooleanKey.OApsAIMINightGrowthEnabled)) CoreUiStrings.yes else CoreUiStrings.no,
            ),
        ),
    )
...
private fun buildSourceSection(preferences: Preferences): AimiControlSectionSnapshot {
    ...
    val sourceValue = AimiStringKey.ActivitySourceMode.entries[sourceMode]
        ?: ApsStrings.pref_aimi_steps_source_disabled
    val ouraConfigured = preferences.get(AimiStringKey.OuraPersonalAccessToken).isNotBlank()
    return AimiControlSectionSnapshot(
        title = ApsStrings.aimi_control_center_sources_title,
        summary = ApsStrings.aimi_control_center_sources_summary,
        details = listOf(
            AimiControlDetail(
                title = AimiStringKey.ActivitySourceMode.title,
                valueText = sourceMode,
                value = sourceValue,
            ),
            AimiControlDetail(
                title = AimiStringKey.OuraPersonalAccessToken.title,
                value = if (ouraConfigured) ApsStrings.aimi_control_center_configured else ApsStrings.aimi_control_center_not_configured,
            ),
        ),
    )
}
...
internal fun loadLatestT3cRuntimeSnapshot(): AimiT3cRuntimeSnapshot {
    ...
    val details = buildList {
        ...
        tick.basalDemandRateUph?.let {
            add(
                AimiControlDetail(
                    title = ApsStrings.aimi_control_center_t3c_basal_demand,
        ...
        tick.appliedRateUph?.let {
            add(
                AimiControlDetail(
                    title = ApsStrings.aimi_control_center_t3c_applied_rate,
        ...
        tick.blocker?.let { blocker ->
            add(
                AimiControlDetail(
                    title = ApsStrings.aimi_control_center_t3c_blocker,
                    valueText = blocker,
                ),
            )
        }
        add(
            AimiControlDetail(
                title = ApsStrings.aimi_control_center_t3c_authority_applied,
                value = if (tick.authorityApplied) CoreUiStrings.yes else CoreUiStrings.no,
            ),
        )
        add(
            AimiControlDetail(
                title = ApsStrings.aimi_control_center_t3c_shadow_only,
                value = if (tick.shadowOnly) CoreUiStrings.yes else CoreUiStrings.no,
            ),
        )
        add(
            AimiControlDetail(
                title = ApsStrings.aimi_control_center_t3c_selected_for_production,
                value = if (tick.selectedForProduction) CoreUiStrings.yes else CoreUiStrings.no,
            ),
        )
        add(
            AimiControlDetail(
                title = ApsStrings.aimi_control_center_t3c_bypass_neutralized,
                value = if (tick.historicalBypassNeutralized) CoreUiStrings.yes else CoreUiStrings.no,
            ),
        )
    }
...
internal fun loadLatestHarmoniaRuntimeSnapshot(): AimiHarmoniaRuntimeSnapshot {
    ...
    val details = buildList {
        ...
        tick.appliedRateUph?.let {
            add(
                AimiControlDetail(
                    title = ApsStrings.aimi_control_center_harmonia_applied_rate,
        ...
        if (tick.smbEligible || tick.smbAppliedToRbtDemand || tick.targetSmbU != null || tick.smbBlocker != null) {
            add(
                AimiControlDetail(
                    title = ApsStrings.aimi_control_center_harmonia_smb_channel,
                    valueText = when {
            ...
            tick.targetSmbU?.let {
                add(
                    AimiControlDetail(
                        title = ApsStrings.aimi_control_center_harmonia_smb_demand,
        ...
        add(
            AimiControlDetail(
                title = ApsStrings.aimi_control_center_harmonia_selected_for_production,
                value = if (tick.selectedForProduction) CoreUiStrings.yes else CoreUiStrings.no,
            ),
        )
        add(
            AimiControlDetail(
                title = ApsStrings.aimi_control_center_harmonia_adds_smb_authority,
                value = if (tick.addsSmbAuthority) CoreUiStrings.yes else CoreUiStrings.no,
            ),
        )
    }
...
private fun boolDetail(
    ...
    AimiControlDetail(
        title = key.controlCenterTitle(),
        value = if (enabled) CoreUiStrings.yes else CoreUiStrings.no,
    )
...
/** Legacy AIMI keys often have no title; map them for Control Center UI. */
internal fun DoublePreferenceKey.controlCenterTitle(): TextRef {
    if (title != TextRef.Literal("")) return title
    return when (this) {
        DoubleKey.autodriveMaxBasal -> ApsStrings.autodrive_max_basal_title
        DoubleKey.meal_modes_MaxBasal -> ApsStrings.meal_modes_max_basal_title
        else -> ApsStrings.aimi_control_center_unlabeled_preference
    }
}

internal fun BooleanPreferenceKey.controlCenterTitle(): TextRef =
    title.takeIf { it != TextRef.Literal("") } ?: ApsStrings.aimi_control_center_unlabeled_preference
