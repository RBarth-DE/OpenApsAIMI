...
open class OpenAPSAIMIPlugin  @Inject constructor(
    ...
    // This plugin still needs Android resource ids (string arrays, R.string literals), so it keeps the
    // Android resolver instead of the TextResolver the migrated plugins use.
    ...
    override suspend fun onStart() {
        ...
        tailDampingTraceJob?.cancel()
        tailDampingTraceJob = rxBus.toFlow(EventPreferenceChange::class)
            .collectResilient(aimiPluginIoScope, aapsLogger, LTag.APS) { event ->
        ...
        physioPreferenceJob?.cancel()
        physioPreferenceJob = rxBus.toFlow(EventPreferenceChange::class)
            .collectResilient(aimiPluginIoScope, aapsLogger, LTag.APS) { event ->
                if (!event.isChanged(BooleanKey.AimiPhysioAssistantEnable.key)) return@collectResilient
                try {
    ...
    override suspend fun onStop() {
        ...
        physioPreferenceJob?.cancel()
        physioPreferenceJob = null
        tailDampingTraceJob?.cancel()
        tailDampingTraceJob = null
    ...
    private var physioPreferenceJob: Job? = null

    /** Diagnostic: bus subscription tracing writes to `OApsAIMISmbTailDamping` (PKPD_TAIL_TRACE). */
    ...
    override suspend fun invoke(initiator: String, tempBasalFallback: Boolean): Unit = withContext(Dispatchers.Default) {
        ...
        if (!hardLimits.checkHardLimits(eff.iCfg.dia, app.aaps.core.interfaces.R.string.profile_dia, hardLimits.diaRange())) return@withContext
        ...
        if (!hardLimits.checkHardLimits(profile.getIsfMgdl("OpenAPSAIMIPlugin"), app.aaps.core.interfaces.R.string.profile_sensitivity_value, HardLimits.LIMIT_ISF)) return@withContext
...
// Construit l?objet attendu par determine_basal
            ...
            determineBasalaimiSMB2.determine_basal(
                ...
                // The old code built an org.json copy of the predictions and `put` it into the tree
                // returned by `json()`. That tree is built fresh on every call, so the write was thrown
                // away, and org.json does not exist on iOS. The RT stored by `with(it)` already carries
                // `predBGs` and `json()` serialises it, so that is the copy that ships.
                if (it.predBGs != null) {
    ...
    /**
     * Pairs a `values` array with its `labels` array, as `StringPreferenceKey.withEntries` wants it.
     *
     * Both sides are plain text read out of the array - not resource ids - so they travel as
     * [TextRef.Literal]. The stored value stays exactly what `values` holds.
     */
    private fun aimiComposeStringArrayMap(@ArrayRes valuesId: Int, @ArrayRes labelsId: Int): Map<String, TextRef> {
        ...
        return values.indices.associate { values[it] to TextRef.Literal(labels[it]) }
    }
            ...
            PreferenceSubScreenDef(
                ...
                items = listOf(
                    ...
                    StringKey.AimiAdvisorProvider.withEntries(
                        mapOf(
                            "OPENAI" to ApsStrings.aimi_prefs_provider_openai,
                            "GEMINI" to ApsStrings.aimi_prefs_provider_gemini,
                            "DEEPSEEK" to ApsStrings.aimi_prefs_provider_deepseek,
                            "CLAUDE" to ApsStrings.aimi_prefs_provider_claude,
                        )
                    ),
            ...
            items = buildList {
                ...
                add(
                    StringKey.AimiAuditorMode.withEntries(
                        mapOf(
                            "AUDIT_ONLY" to ApsStrings.aimi_auditor_mode_audit_only,
                            "SOFT_MODULATION" to ApsStrings.aimi_auditor_mode_soft_modulation,
                            "HIGH_RISK_ONLY" to ApsStrings.aimi_auditor_mode_high_risk_only,
                        )
                    )
                )
