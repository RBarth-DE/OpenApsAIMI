...
class IntelligoPlugin(
    ...
    // Built on first read, not in the constructor. `toUri()` calls `Uri.parse`, which only works on a
    // device: a plain JVM test gets the stub Android jar, where it returns null and the constructor
    // throws. The DI graph tests build every plugin, so the constructor must not touch Android.
    private val contentUri: Uri by lazy { "content://$AUTHORITY/$TABLE_NAME".toUri() }
