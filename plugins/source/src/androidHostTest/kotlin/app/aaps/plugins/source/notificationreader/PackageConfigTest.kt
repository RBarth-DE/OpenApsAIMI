...
class PackageConfigTest {
    ...
    fun `bundled notification_reader_packages maps d1plus and dexcomone to OnePlus`() {
        // The Android source set moved to `androidMain` when this module became multiplatform, so
        // the asset is no longer under `src/main`.
        val asset = File("src/androidMain/assets/notification_reader_packages.json")
