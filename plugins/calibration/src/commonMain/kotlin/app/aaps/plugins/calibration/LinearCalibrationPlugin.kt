...
@MetroIntKey(710)
@ContributesBinding(AppScope::class, binding = binding<Calibration>())
...
class LinearCalibrationPlugin(
    ...
    private val profileUtil: ProfileUtil) : PluginBase(
    PluginDescription()
    ...
    /** Reason last told to the user (one of the `cal_notify_*` texts), so the same reason is not
     *  re-announced on every scan. Null once resolved or not yet checked. */
    @Volatile
    private var lastHealthMessage: TextRef? = null

    /**
     * Guards [lagPairedSensorValues]. A plain map under one lock, the same pattern as
     * `ProfileFunctionImpl`: `ConcurrentHashMap` is a JVM only type, and this class is in commonMain
     * because it also has to compile for iOS.
     */
    private val lagPairedLock = AapsLock()
     ...
     */
    private val lagPairedSensorValues = mutableMapOf<Long, Double>()
    ...
    private suspend fun entriesForFit(sessionStart: Long, now: Long): List<CAL> {
        if (lagPairedSession != sessionStart) {
            ...
            lagPairedLock.withLock { lagPairedSensorValues.clear() }
        }
    ...
    private suspend fun lagPaired(entry: CAL, now: Long): CAL {
        ...
        lagPairedLock.withLock { lagPairedSensorValues[entry.id] }?.let { return entry.copy(sensorMgdlAtPairing = it) }
        ...
        lagPairedLock.withLock { lagPairedSensorValues[entry.id] = paired }
        aapsLogger.debug(LTag.GLUCOSE) {
    ...
    private suspend fun checkCalibrationHealthAndNotify(sessionStart: Long?, now: Long) {
        ...
        if (message == lastHealthMessage) return
        lastHealthMessage = message
        notificationManager.post(id = NotificationId.CALIBRATION_HEALTH, text = rh.gs(message))
    }
