...
internal class GraphViewModelTest {
    ...
    fun setUp() {
        ...
        // The fork's view model reads four more keys than the two above: the graph look and the
        // display units. Two of them feed a field initializer whose type is non-null, and the other
        // two are observed in init{}, so an unstubbed mock throws instead of returning a default.
        whenever(preferences.get(StringKey.OverviewVicoBgReadingTint)).thenReturn("theme")
        whenever(preferences.get(StringKey.OverviewVicoChartBackdrop)).thenReturn("theme")
        whenever(preferences.observe(StringKey.OverviewVicoBgReadingTint)).thenReturn(MutableStateFlow("theme"))
        whenever(preferences.observe(StringKey.OverviewVicoChartBackdrop)).thenReturn(MutableStateFlow("theme"))
        whenever(preferences.observe(StringKey.GeneralUnits)).thenReturn(MutableStateFlow("mg/dl"))
        // fullWindow = false: the overview behaviour, where the axis hugs the data.
        sut = graphViewModel(cache, false)
    }

    /**
     * The graph view model with every dependency mocked. The fork's class carries the whole overview
     * state, so it takes twenty-two dependencies; only the ones a test names are stubbed, and the
     * rest stay bare mocks because these tests read the axis and the chart look, nothing else.
     */
    private fun graphViewModel(cache: OverviewDataCache, fullWindow: Boolean): GraphViewModel =
        GraphViewModel(
            cache = cache, fullWindow = fullWindow, graphConfigRepository = graphConfigRepository,
            aapsLogger = aapsLogger, preferences = preferences, dateUtil = dateUtil, rh = rh,
            iobCobCalculator = mock(), decimalFormatter = mock(), loop = mock(), config = mock(),
            persistenceLayer = mock(), constraintChecker = mock(), profileFunction = mock(),
            processedDeviceStatusData = mock(), profileUtil = mock(), activePlugin = mock(),
            automation = mock(), processedTbrEbData = mock(), auditorStateProvider = mock(),
            mealHypothesisHistorySource = mock(), ch = mock()
        )
    ...
    fun `predictions alone do not pull the left edge of the axis up to now`() = runTest {
        ...
        val vm = graphViewModel(cache, false)
