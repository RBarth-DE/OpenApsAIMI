...
internal class EversenseCalibrationDialogViewModelTest {
    ...
    fun setUp() {
        ...
        whenever(rh.gs(eq(UiStrings.eversense_calibration_send_failed))).thenReturn("send failed")
    ...
    fun `a successful calibration emits accepted and logs a user entry`() = runTest(testDispatcher) {
        ...
        // `note` is a matcher too: the call site leaves it out, so Kotlin fills the default and
        // Mockito would see one raw value among three matchers ("4 matchers expected, 3 recorded").
        verify(uel).log(action = any(), source = any(), note = any<String>(), value = any())
    }
}
