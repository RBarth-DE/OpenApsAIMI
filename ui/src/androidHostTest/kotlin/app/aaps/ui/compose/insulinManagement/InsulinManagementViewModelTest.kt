...
internal class InsulinManagementViewModelTest {
    ...
    fun setUp() {
        ...
        // loadData() asks for the limit ranges to recover an edited inhaled (Afrezza) peak. A bare
        // mock answers null, and the non-null Kotlin range type turns that into an NPE.
        whenever(hardLimits.peakRange()).thenReturn(HardLimits.MIN_PEAK..HardLimits.MAX_PEAK)
        whenever(hardLimits.peakRangeInhaled()).thenReturn(HardLimits.MIN_PEAK_INHALED..HardLimits.MAX_PEAK_INHALED)
        whenever(hardLimits.diaRange()).thenReturn(4.0..9.0)        // hard limits for an adult
        whenever(hardLimits.diaRangeInhaled()).thenReturn(1.5..4.0) // inhaled band, see HardLimits
