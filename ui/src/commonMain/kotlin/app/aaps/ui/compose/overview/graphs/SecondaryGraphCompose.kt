...
fun SecondaryGraphCompose(
    ...
    val processedBasalProfile = remember(basalData, hasIob, stableTimeRange) {
        ...
        // Fork: only the IOB graph flips the basal overlay (negate: y=0 at top, -maxBasal at bottom),
        // so a basal-only graph keeps positive values.
    ...
    val primaryYValues = remember(
        processedIob, processedCob, processedCarbs, processedSimpleSeries, processedDevSlopeMin, processedDeviationLines, processedMhbPts, visibleMinX, visibleMaxX
    ) {
        windowedPrimaryY(visibleMinX, visibleMaxX, processedIob, processedCob.first, processedCarbs, processedSimpleSeries, processedDevSlopeMin, processedDeviationLines, processedMhbPts)
    }
...
private fun rememberIobValueMarker(
    ...
    // DateUtil rather than SimpleDateFormat: the same time string the rest of the screen shows,
    // and it is the one formatter that works on every target.
    val dateUtil = LocalDateUtil.current
