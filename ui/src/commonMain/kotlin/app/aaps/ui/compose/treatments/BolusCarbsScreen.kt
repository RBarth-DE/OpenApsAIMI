            ...
            mealLink.bolus?.let { bolus ->
                if (bolus.isValid || showInvalidated) {
                    Row(
                        ...
                        Text(
                            text = when {
                                isAfrezzaDose                 -> stringResource(CoreUiStrings.afrezza_type_shortname)
                                bolus.type == BS.Type.SMB     -> stringResource(CoreUiStrings.smb_shortname)
                                bolus.type == BS.Type.NORMAL  -> stringResource(CoreUiStrings.careportal_mealbolus)
                                bolus.type == BS.Type.PRIMING -> stringResource(CoreUiStrings.prime_fill)
                                else                          -> stringResource(CoreUiStrings.careportal_mealbolus)
                            },
                            fontSize = 11.sp,
                            color = GlassColors.textMuted(isDark)
                        )
