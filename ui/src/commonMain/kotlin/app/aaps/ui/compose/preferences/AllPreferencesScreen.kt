 ...
 *   Null on a platform that has no XML preference screen; the toolbar button is then not shown.
 */
...
fun AllPreferencesScreen(
    ...
    // Fork: the drilled plugin sub-screen closes the same way; the two never show at once.
    NavigationBackHandler(
        state = rememberNavigationEventState(NavigationEventInfo.None),
        isBackEnabled = drilledSubScreen != null,
        onBackCompleted = { drilledSubScreen = null }
    )
                            ...
                            onOpenLegacyXmlPreferences?.let { openLegacy ->
                                IconButton(onClick = openLegacy) {
                                    Icon(
                                        ...
                                        contentDescription = stringResource(CoreUiStrings.legacy_xml_preferences)
                                    )
                                }
                            }
                        }
