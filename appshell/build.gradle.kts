 ...
 */
/**
 * Plugin modules that are Android libraries, not multiplatform ones: the fork's three native CGM
 * plugins and `libkeks`, the `jamorham.keks` support library `:plugins:dexcom_oneplus` uses.
 *
 * An Android library publishes no variant for the jvm or the Apple targets, so these four cannot be
 * on the `commonMain` classpath - `:appshell:jvmTest` and the iOS targets cannot resolve them there.
 * They are `api` dependencies of `androidMain` instead, so the Android classpath of `:app`, and the
 * DI graph built from it, stay exactly as they were when the wildcard below covered them.
 */
val androidOnlyPlugins = setOf(
    ...
    sourceSets {
        commonMain {
            dependencies {
                ...
                // The Android-only ones are added on the Android side instead, see androidOnlyPlugins.
                rootProject.subprojects
                    .filter { it.path.startsWith(":plugins:") && it.buildFile.exists() }
                    .filterNot { it.path in androidOnlyPlugins }
        ...
        androidMain {
            dependencies {
                // See androidOnlyPlugins above. `:plugins:source` also depends on the three CGM
                // plugins, but from its own androidMain and as `implementation`, so that edge does
                // not carry them into the app's DI graph - these `api` lines do.
                androidOnlyPlugins.forEach { api(project(it)) }
