...
fun NavGraphBuilder.appNavGraph(
    ...
    /**
     * Opens the classic XML preference screen, the `PreferenceFragment` tree that came before the
     * Compose settings screens. It shows the same data as the Compose screen, in the old look.
     *
     * Null means "this platform has no such screen" and the toolbar button is then not drawn at all.
     * Android passes a real handler; iOS and the desktop have no XML preference screen to open, so
     * they pass nothing and their users do not see a button that leads nowhere.
     *
     * @param pluginSimpleName the plugin whose preference screen to open, or null for the whole
     *   preference tree.
     */
    onOpenLegacyXmlPreferences: ((pluginSimpleName: String?) -> Unit)? = null,
    /**
    ...
    // The whole preference tree, with no plugin chosen. Null stays null, so a platform without the
    // XML screen passes nothing on and the settings toolbar simply has no legacy button.
    val openLegacyPreferences: (() -> Unit)? =
        onOpenLegacyXmlPreferences?.let { open -> { open(null) } }
    ...
    composable(AppRoute.Preferences.route) {
        AllPreferencesScreen(
            ...
            configBuilder = configBuilder,
            onBackClick = { navController.safePopBackStack() },
            onOpenLegacyXmlPreferences = openLegacyPreferences
        )
    }
