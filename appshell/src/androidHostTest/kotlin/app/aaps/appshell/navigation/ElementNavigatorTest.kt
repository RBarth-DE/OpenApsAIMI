...
class ElementNavigatorTest {
    ...
    fun `every element type either navigates, acts, or is a declared no-op`() {
        ...
        // PUMP and BGSOURCE go through the active plugin, which needs a plugin list rather than a
        // bare mock: the mock returns null for the active pump and the active bg source, and both
        // branches cast that to `PluginBase`.
        val exercisedElsewhere = setOf(ElementType.PUMP, ElementType.BGSOURCE)
