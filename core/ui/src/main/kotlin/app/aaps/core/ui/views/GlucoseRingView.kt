...
/**
 * Ring that shows the current BG in the center + two small texts at the bottom.
 * Supports a "nose" triangle (0..180° where 90° = flat/right).
 *
 * NOTE: Size is controlled by layout_width/height. We do NOT clamp the radius,
 * so the ring actually grows when the view grows.
 */
class GlucoseRingView @JvmOverloads constructor(
    ...
    private fun computeRingColor(bgMgdl: Int?): Int {
        ...
        if (!useSteppedColors) {
            return GlucoseColorMapper.colorFor(v)
        }
    ...
    private fun drawNose(canvas: Canvas, cx: Float, cy: Float, radius: Float) {
        ...
        //Log.d(LTag.BGSOURCE.name ,"RBarth: NOSE: noseAngleDeg=${noseAngleDeg} tipR=${tipR} baseR=${baseR}")
