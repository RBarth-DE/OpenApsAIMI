 ...
 * 2. [unitFormat] set → stringResource with value, rounded first when the template needs a whole number
 * 3. unitLabel set → "formatted_value unitLabel"
 * 4. Plain → valueFormat.format(value)
 *
 * @param unitFormat The unit's template together with its rounding, from
 *   [app.aaps.core.keys.UnitType.unitFormat]. One value rather than a template plus a flag, so the
 *   two cannot be made to disagree - a `%d` template handed a Double throws.
...
fun formatSliderDisplayValue(
    ...
    return when {
        ...
        unitFormat != null             -> {
            if (unitFormat.asWholeNumber) stringResource(unitFormat.ref, value.roundToInt())
            else stringResource(unitFormat.ref, value)
        }
