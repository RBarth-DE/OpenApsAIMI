...
fun AdaptiveIntPreferenceItem(
    ...
    // An Int key is always whole, and UnitType says so for every unit an IntKey declares, so this
    // carries the rounding the slider needs. No hardcoded `formatAsInt = true` any more.
    ...
    if (hasValidRange) {
        Column(
            ...
            TextWithSyncBadge(
                text = preferenceDisplayTitle(effectiveTitle, intKey.key),
                key = intKey,
            ...
            PreferenceSliderWithButtons(
                ...
                dialogLabel = preferenceDisplayTitle(effectiveTitle, intKey.key),
