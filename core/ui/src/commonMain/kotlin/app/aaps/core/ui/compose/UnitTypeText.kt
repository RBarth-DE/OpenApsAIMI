...
import app.aaps.core.keys.isWholeNumber
 ...
 *
 * Private on purpose. A template is only half of what a caller needs - the other half is the
 * rounding in [unitFormat] - and taking it alone is exactly how a `%d` string came to be handed a
 * Double. Every call site wants [unitFormat] instead.
 *
 * Still a raw resource id, unlike the two above, because the slider applies it to whatever value
 * the user drags to - the arguments are not known here.
 */
private fun UnitType.valueFormat(): TextRef? = when (this) {
    UnitType.NONE         -> null
...
/**
 * A format template together with the rounding its placeholders need.
 *
 * The two are one fact, not two: `%d` takes an Int and `%f` takes a Double. While they travelled as
 * separate parameters, a caller could take the template and forget the rounding, and nothing said
 * so - the mistake only showed up at runtime, as IllegalFormatConversionException, when a value
 * reached the format string. Keeping them in one value means there is no half of it to take.
 */
data class UnitFormat(val ref: TextRef, val asWholeNumber: Boolean)

/**
 * The format template for this unit, with the rounding it needs. Null when the unit has no template.
 *
 * Prefer this over [valueFormat] wherever the caller would otherwise have to decide about rounding
 * as well.
 */
fun UnitType.unitFormat(): UnitFormat? =
    valueFormat()?.let { UnitFormat(it, isWholeNumber()) }
