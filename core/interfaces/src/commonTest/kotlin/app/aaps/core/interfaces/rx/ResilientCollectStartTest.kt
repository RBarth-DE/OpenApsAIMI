 ...
 *
 * In `commonTest` rather than `androidHostTest`: the class under test is shared code, and the two
 * behaviours it pins are the same on every target. The only change the move needed was the logger -
 * `mock<AAPSLogger>()` became [RecordingAAPSLogger], which nothing in this test writes to.
 */
class ResilientCollectStartTest {

    private val aapsLogger: AAPSLogger = RecordingAAPSLogger()
    ...
    fun `undispatched start receives an item emitted immediately after subscribing`() {
        ...
        assertEquals(1, bus.subscriptionCount.value)
    }
