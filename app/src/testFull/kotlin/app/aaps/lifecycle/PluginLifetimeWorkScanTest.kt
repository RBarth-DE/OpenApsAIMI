...
class PluginLifetimeWorkScanTest {
     ...
     *
     * Five were added on 2026-09-21, when the merge from upstream `dev` brought this scan: two in
     * `LoopPlugin` and three in `EversensePlugin`, all of them written by the fork long before the
     * scan existed. Each one was read before it was written down, and the decision to record them
     * rather than change the plugins in a merge is in `docs/MERGE_DEV_2026-09-21.md`.
     */
    private val survivesStop: Map<String, String> = mapOf(
        "LoopPlugin#executeInvokeInternal" to
            "appScope.launch { delay(1000); invoke(\"tempBasalFallback\") } reschedules the loop a second later, so it " +
                "lands inside or just after a restart window and can queue pump commands against a driver being torn " +
                "down. The worst one here. (Recorded as LoopPlugin#invoke before the fork moved this code, 2026-09-21.)",
        "LoopPlugin#startPeriodicLoop" to
            "The tick itself is held in periodicLoopJob and onStop cancels it. Its inner " +
                "appScope.launch { invoke(\"PeriodicApsMaxSmbFrequency\") } is not tracked, so that one run survives a " +
                "stop. Same shape as executeInvokeInternal above, and no worse than it.",
        "EversensePlugin#mainHandler" to
            "A main-thread Handler created once, never torn down: onStop does not remove its callbacks. " +
                "Recorded 2026-09-21, merge from upstream dev.",
        "EversensePlugin#showDeviceSelectionDialog" to
            "mainHandler.postDelayed runs the scan timeout - stopScan, dismiss and then a result dialog - even if the " +
                "plugin was stopped while the user was still scanning.",
        "EversensePlugin#startOfficialAppReleaseReconnectLoop" to
            "Re-posts itself every 5 minutes while it cannot connect. Dead code today - releaseForOfficialApp is never " +
                "set to true and nothing calls this method - so it cannot run, and it is listed here for the day " +
                "someone arms that flag.",
