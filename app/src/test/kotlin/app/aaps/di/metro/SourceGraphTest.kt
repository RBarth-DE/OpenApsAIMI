...
class SourceGraphTest {
    ...
    @Test
    fun `every source worker is registered`() {
        assertThat(graph().workerCreators.keys).containsExactly(
            ...
            // FORK: Ottai has its own worker and does not share SyaiWorker (see Intents.OTTAI_APP in
            // DataReceiver - the two apps send different payloads).
            OttaiWorker::class,
