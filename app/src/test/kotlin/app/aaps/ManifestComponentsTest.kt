 ...
 *
 * An `<activity-alias>` is checked through its `android:targetActivity`, not its `android:name`: the
 * alias name is a component name the platform looks up by intent action, and it is allowed to name no
 * class at all. `ViewPermissionUsageActivity` is the name Health Connect documents for the Android 14+
 * "Manage Access" link, and it is deliberately package-less - the alias itself is the component, and
 * `targetActivity` says what actually runs.
 */
class ManifestComponentsTest {
    ...
    fun everyDeclaredComponentClassExists() {
        ...
        // An alias declares no class of its own - check the activity it points at.
        val aliases = document.getElementsByTagName("activity-alias")
        for (i in 0 until aliases.length) {
            val element = aliases.item(i) as? Element ?: continue
            val target = element.getAttribute("android:targetActivity").takeIf { it.isNotBlank() } ?: continue
            if (target.startsWith(".")) continue
            checked++
            val exists = runCatching { Class.forName(target, false, javaClass.classLoader) }.isSuccess
            if (!exists) missing.add("<activity-alias> targetActivity=$target")
        }
