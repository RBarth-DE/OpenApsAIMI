...
class ImportExportPrefsImpl(
    ...
    /**
     * The SAF file [prepareExport] created, held until [executeExport] writes the settings into it.
     * Zeroed at the start of every prepare, so a failed prepare cannot leave the previous export's
     * file behind to be overwritten by the next execute.
     */
    ...
    override fun prepareExport(): ExportPreparation? {
        ...
        // Kept for executeExport(), which the interface calls without a file: prepare and execute are
        // two steps of one export, and the UI has no Android file handle to pass back.
