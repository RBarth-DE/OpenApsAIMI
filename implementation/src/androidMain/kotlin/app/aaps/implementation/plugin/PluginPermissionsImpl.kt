...
class PluginPermissionsImpl(
    ...
    private fun globalPermissions(context: Context): List<PermissionGroup> = buildList {
        ...
        // Fork: DND override for URGENT medical alarms. Only surfaced when the user keeps the
        // override on (default): a bypass-DND notification channel is honored by the OS only once
        // the user grants notification-policy access via Settings.ACTION_NOTIFICATION_POLICY_ACCESS_SETTINGS.
        if (preferences.get(BooleanKey.AlertOverrideDoNotDisturb)) {
            add(
                PermissionGroup(
                    permissions = listOf(Manifest.permission.ACCESS_NOTIFICATION_POLICY),
                    rationaleTitle = TextRef.AndroidRes(R.string.permission_dnd_title),
                    rationaleDescription = TextRef.AndroidRes(R.string.permission_dnd_description),
                    special = true,
                )
            )
        }
