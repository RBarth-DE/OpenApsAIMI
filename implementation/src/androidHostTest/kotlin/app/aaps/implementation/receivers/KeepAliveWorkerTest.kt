...
class KeepAliveWorkerTest : TestBaseWithProfile() {
    ...
    /**
     * The worker asks the database which running mode was active, not the live loop: after a process
     * restart the in-memory loop state can still be empty while the stored record already says what
     * the user was running. So the stub belongs on the persistence layer.
     */
    private suspend fun stubRunningMode(mode: RM.Mode) {
        whenever(persistenceLayer.getRunningModeActiveAt(any())).thenReturn(RM(timestamp = now, mode = mode, duration = 0))
    }
    ...
    fun `doWorkAndLog reschedules the 5 and 10 minute runs when triggered by the periodic run`() = runTest {
        ...
        stubRunningMode(RM.Mode.OPEN_LOOP)
    ...
    fun `checkPump does nothing if mode is DISCONNECTED_PUMP`() = runTest {
        // Arrange
        worker = createWorker()
        stubRunningMode(RM.Mode.DISCONNECTED_PUMP)
        testPumpPlugin.lastData = now - T.mins(20).msecs()
    ...
    fun `checkAPS schedules device status upload if BG is missing`() = runTest {
        // Arrange
        worker = createWorker()
        stubRunningMode(RM.Mode.CLOSED_LOOP)
    ...
    fun `databaseCleanup does NOT run if it was run less than a day ago`() = runTest {
        ...
        // Assert
        verify(persistenceLayer, never()).cleanupDatabase(any(), any(), any())
    }
    ...
    fun `databaseCleanup runs and records the timestamp when last run was over a day ago`() = runTest {
        ...
        whenever(persistenceLayer.cleanupDatabase(any(), any(), any())).thenReturn("done")
