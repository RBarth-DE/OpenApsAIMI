...
class GlucoseStatusTestAutoIsf : TestBaseWithProfile() {
    ...
    /**
     * The fork's calculator also reads the FSL calibration preferences, and building it by hand here
     * means nothing fills its `@Inject` fields - so `preferences` is set on the instance. A mock
     * returns 0 / 0L / false for the three calibration keys, which leaves SMB not stopped.
     */
    private fun autoIsfCalculator(): GlucoseStatusCalculatorAutoIsf =
        GlucoseStatusCalculatorAutoIsf(aapsLogger, iobCobCalculator, dateUtil, deltaCalculator)
            .also { it.preferences = preferences }

    /**
     * The fork's calculator reads the raw (unbucketed) readings as well as the bucketed ones: the
     * Libre calibration block works on the raw list. Mockito answers an unstubbed list call with an
     * empty list and the calculator reads element 0 of it, so both calls must be stubbed.
     *
     * The raw list only feeds the Libre branch and one log line here, so the same values are mapped
     * over, with `raw` taken from the value.
     */
    private fun storeReadings(records: MutableList<InMemoryGlucoseValue>) {
        whenever(autosensDataStore.getBucketedDataTableCopy()).thenReturn(records)
        whenever(autosensDataStore.getBgReadingsDataTableCopy()).thenReturn(
            records.map {
                GV(
    ...
    @Test fun calculateValidGlucoseStatusAutoIsf() {
        storeReadings(generateValidBgData())
        val glucoseStatus = autoIsfCalculator().getGlucoseStatusData(false)!!
        ...
        // The fork's parabola fit starts only once the window is longer than `fslMinDur` (15 minutes
        // for a normal sensor), and its fit quality floor was dropped. So the 15 minute window that
        // upstream reports is skipped here and the fit covers the whole 35 minute series: the numbers
        // from `parabolaMinutes` down describe that fit. The values they replaced (15 minutes, and a
        // fit quality of 1.0) described the 15 minute window.
        assertThat(glucoseStatus.parabolaMinutes).isWithin(0.1).of(35.0) // parabola size records in minutes
        assertThat(glucoseStatus.deltaPl).isWithin(0.1).of(-2.5714) // last delta
        assertThat(glucoseStatus.deltaPn).isWithin(0.1).of(-2.7857) // next delta
        assertThat(glucoseStatus.bgAcceleration).isWithin(0.01).of(-0.2143) // glucose acceleration
        ...
        assertThat(glucoseStatus.corrSqu).isWithin(0.001).of(0.9643) // parabola fit quality
    }

    @Test fun oneRecordShouldProduceZeroDeltas() {
        storeReadings(generateOneCurrentRecordBgData())
        val glucoseStatus = autoIsfCalculator().getGlucoseStatusData(false)!!
        assertThat(glucoseStatus.glucose).isWithin(0.001).of(214.0)
    ...
    @Test fun insufficientDataShouldReturnNull() {
        storeReadings(generateInsufficientBgData())
        val glucoseStatus = autoIsfCalculator().getGlucoseStatusData(false)
        assertThat(glucoseStatus).isNull()
    }

    @Test fun oldDataShouldReturnNull() {
        storeReadings(generateOldBgData())
        val glucoseStatus = autoIsfCalculator().getGlucoseStatusData(false)
        assertThat(glucoseStatus).isNull()
    }
    ...
    @Test fun returnOldDataIfAllowed() {
        storeReadings(generateOldBgData())
        val glucoseStatus = autoIsfCalculator().getGlucoseStatusData(true)
