...
/**
 * The graph time window, bound for the whole app.
 *
 * Only the window is here. The graph data itself belongs to the Compose path
 * (`OverviewDataCache`) and to the old overview screen's own implementation in `:plugins:main`.
 */
...
class OverviewDataImpl() : OverviewData {
    ...
    // first predictions-prep run see sensible values. Rounded to the next
    // full hour plus the old GraphView 100 ms nudge, to avoid axis-label rounding.
