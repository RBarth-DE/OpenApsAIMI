...
class CgmSourceTransactionTest {
    ...
    fun `keeps local value when NS data arrives for a fresh local reading`() = runTest {
        ...
        val transaction = CgmSourceTransaction(listOf(gv), emptyList(), null, fromNsClient = true)
        transaction.database = database
        val result = transaction.run()
    ...
    @Test
    fun `keeps local value when NS data arrives for an old local reading`() = runTest {
        ...
        // The age of the reading plays no part: an echo from Nightscout never overwrites a value
        // this app stored itself, and the reading only gains the Nightscout id.
        assertThat(result.updated).isEmpty()
        assertThat(result.updatedNsId).hasSize(1)
        assertThat(existing.value).isEqualTo(71.0)
    ...
    fun `updates value when local data arrives for a fresh local reading`() = runTest {
        ...
        val transaction = CgmSourceTransaction(listOf(gv), emptyList(), null, fromNsClient = false)
        transaction.database = database
        val result = transaction.run()
