...
internal interface CarbsDao : TraceableDao<Carbs> {
    ...
    @Query("SELECT * FROM $TABLE_CARBS WHERE isValid = 1 AND referenceId IS NULL AND timestamp <= :now ORDER BY timestamp DESC LIMIT 1")
    fun getMostRecentCarbByDate(now: Long): Carbs?
}
