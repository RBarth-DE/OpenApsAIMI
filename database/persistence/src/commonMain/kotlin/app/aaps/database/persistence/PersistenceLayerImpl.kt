...
class PersistenceLayerImpl(
    ...
    override fun getAutoIsfValuesFromTime(from: Long): List<AIV> =
        runBlocking { repository.getAutoIsfValuesFromTime(from).map { it.fromDb() } }

    override fun getAutoIsfValuesFromTimeToTime(startTime: Long, endTime: Long): List<AIV> =
        runBlocking { repository.getAutoIsfValuesFromTimeToTime(startTime, endTime).map { it.fromDb() } }

    override suspend fun insertOrUpdateAutoIsfValues(autoIsfValues: AIV): PersistenceLayer.TransactionResult<AIV> = withContext(aapsIoDispatcher) {
        try {
